...
const SideBar = ({ setSavedOutfits }) => {
    ...
    const handleBookmark = () => {
        ...
        let newSavedOutfitIds = savedOutfitIds;

        if (saved) {
            newSavedOutfitIds = savedOutfitIds.filter(id => id !== currentOutfitId);
            setBookmarks(bookmarks - 1);
        } else {
            newSavedOutfitIds = [...savedOutfitIds, currentOutfitId];
            setBookmarks(bookmarks + 1);
        }
        setSavedOutfitIds(newSavedOutfitIds);
        setSaved(!saved);
        setSavedOutfits(newSavedOutfitIds);
    };
